...
PY
  ...
  failure_stage=metadata-$artifact
  metadata_digest=$(
    "$python" - "$metadata" <<'PY'
import json
import re
...
value = json.loads(Path(sys.argv[1]).read_bytes(), object_pairs_hook=pairs)
digest = value.get("containerimage.digest") if isinstance(value, dict) else None
if not isinstance(digest, str) or re.fullmatch(r"sha256:[0-9a-f]{64}", digest) is None:
    raise SystemExit(1)
print(digest)
PY
  ) || fail "$artifact build metadata is invalid" 70

  failure_stage=index-$artifact
  index=$artifact_dir/index.json
  "$docker" --config "$docker_config" buildx imagetools inspect --raw "$tagged" >"$index" || \
    fail "cannot read the pushed $artifact index" 70
  descriptor_pair=$(
    "$python" - "$metadata_digest" "$index" <<'PY'
import hashlib
import json
import re
import sys
...
for item in manifests:
    if not isinstance(item, dict) or digest_pattern.fullmatch(item.get("digest", "")) is None:
        raise SystemExit(1)
...
if len(runnable) != 1:
    raise SystemExit(1)
child = runnable[0]["digest"]
if len(attestations) != 1 or any(
    item["annotations"].get("vnd.docker.reference.digest") != child for item in attestations
):
    raise SystemExit(1)
print(child, attestations[0]["digest"])
PY
  ) || fail "$artifact registry index is invalid" 70
  set -- $descriptor_pair
  [ "$#" -eq 2 ] || fail "$artifact registry descriptor set is invalid" 70
  child_digest=$1
  attestation_digest=$2

  failure_stage=manifest-$artifact
  manifest=$artifact_dir/amd64-manifest.json
  "$docker" --config "$docker_config" buildx imagetools inspect --raw "$repository@$child_digest" \
    >"$manifest" || fail "cannot read the pushed $artifact manifest" 70
  attestation_manifest=$artifact_dir/attestation-manifest.json
  "$docker" --config "$docker_config" buildx imagetools inspect --raw "$repository@$attestation_digest" \
    >"$attestation_manifest" || fail "cannot read the pushed $artifact attestation manifest" 70

  failure_stage=visibility-$artifact
  visibility=$("$gh" api --hostname github.com \
    "/orgs/milkinfrastructure/packages/container/$package" --jq '.visibility') || \
    fail "cannot verify $package visibility" 77
  [ "$visibility" = private ] || fail "$package is not private" 77

  failure_stage=verify-$artifact
  gateway_binding=-
  [ "$gateway_required" = no ] || gateway_binding=$gateway_image
  immutable=$(
    "$python" - \
      "$artifact" "$repository" "$tagged" "$commit" "$gateway_binding" \
      "$source_epoch" \
      "$metadata_digest" "$child_digest" "$manifest" "$attestation_digest" \
      "$attestation_manifest" "$artifact_dir/build-log.json" "$artifact_dir/receipt.json" \
      "$artifact_dir/admission.json" "$context_sha256" \
      "$BUILDKIT_IMAGE" "$BUILDKIT_VERSION" "$DOCKERFILE_FRONTEND" <<'PY'
import hashlib
import json
import re
import sys
...
(
    artifact, repository, tagged, commit, gateway, source_epoch, index_digest, child_digest,
    manifest_path, attestation_digest, attestation_path, log_path, receipt_path,
    admission_path, context_sha256, buildkit, buildkit_version, frontend,
) = sys.argv[1:]
digest_pattern = re.compile(r"sha256:[0-9a-f]{64}\Z")
...
if not isinstance(config, dict) or digest_pattern.fullmatch(config.get("digest", "")) is None:
    raise SystemExit(1)
if not isinstance(manifest.get("layers"), list):
    raise SystemExit(1)
attestation_raw = Path(attestation_path).read_bytes()
if "sha256:" + hashlib.sha256(attestation_raw).hexdigest() != attestation_digest:
    raise SystemExit(1)
attestation = json.loads(attestation_raw, object_pairs_hook=pairs)
if not isinstance(attestation, dict) or attestation.get("schemaVersion") != 2:
    raise SystemExit(1)
subject = attestation.get("subject")
if subject is not None and (
    not isinstance(subject, dict) or subject.get("digest") != child_digest
):
    raise SystemExit(1)
...
attestations = []
for layer in layers:
    if not isinstance(layer, dict) or digest_pattern.fullmatch(layer.get("digest", "")) is None:
        raise SystemExit(1)
    annotations = layer.get("annotations")
    if layer.get("mediaType") == "application/vnd.in-toto+json" and isinstance(annotations, dict):
        predicate = annotations.get("in-toto.io/predicate-type")
        if isinstance(predicate, str):
            attestations.append({
                "layer_sha256": layer["digest"].removeprefix("sha256:"),
                "predicate_type": predicate,
            })
attestations.sort(key=lambda item: (item["predicate_type"], item["layer_sha256"]))
if [item["predicate_type"] for item in attestations] != [
    "https://slsa.dev/provenance/v1",
    "https://spdx.dev/Document",
]:
    raise SystemExit(1)
log = json.loads(Path(log_path).read_bytes(), object_pairs_hook=pairs)
if not isinstance(log, dict) or log.get("schema_version") != "milk.content-free-build-log.v1":
    raise SystemExit(1)
...
receipt = {
    ...
    "config_sha256": config["digest"].removeprefix("sha256:"),
    "gateway_image_reference": None if gateway == "-" else gateway,
    "build_log": log,
    "visibility": "private",
...
Path(admission_path).write_text(admission_raw, encoding="utf-8")
