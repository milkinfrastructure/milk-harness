...
Both environments require exact digest references for `milk-gateway`, `milk-jobs`, `milk-student`, and `milk-teacher-gpt-oss`. Provider jobs also require `MILK_IMAGE_RELEASE_SHA256`; the jobs image loads and verifies that immutable release and each image admission from evidence before provider setup.
